...
export type IForm = {
  ...
  isValid: boolean
  handleSubmit: (onSubmit: Function) => (e: React.FormEvent) => void
}
...
export const useForm = (initialFields: ICustomObject): IForm => {
    ...
    (fields, [name, value]: any[]) => {
      ...
      const field = {
        [name]: {
          ...
          touched: false,
          setState: (value: React.ChangeEvent<HTMLInputElement>) =>
  ...
  const [fields, setState] = React.useState<ICustomObject>(form)
  const [isValid, setFormValid] = React.useState<boolean>(true)
